...
import { getFMUManifest, runFMUTest, listResources, uploadResource, deleteResource, repairResource } from '../api'
...
export default function FMUDetail() {
  ...
  async function handleRepairDataFile(filename: string) {
    if (!typeName) return
    setRepairingFile(filename)
    setRepairMessage(null)
    try {
      const result = await repairResource(typeName, filename)
      setRepairMessage(result.message)
      // Refresh file list to get updated validation
      const r = await listResources(typeName)
      setDataFiles(r.resources)
    } catch (err) {
      ...
      {manifest && (
        <div className="space-y-5">
          ...
          <section className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            ...
            {repairMessage && (
              <div className="mb-3 bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-xs text-gray-300 flex items-start gap-2">
                ...
                <button
                  ...
                  className="ml-auto p-0.5 text-gray-500 hover:text-gray-300 flex-shrink-0"
                >
                ...
                {dataFiles.map((f) => (
                  <div key={f.name}>
                    <div className="flex items-center gap-2 bg-gray-800/60 rounded-lg px-3 py-2">
                      ...
                      {f.validation && f.validation.format_valid && f.validation.has_header && (
                    ...
                    {/* Validation info box for .data files with issues */}
                    {f.validation && (!f.validation.format_valid || !f.validation.has_header) && (
                      <div className="mt-1.5 bg-amber-950/40 border border-amber-800/60 rounded-lg px-4 py-3">
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium text-amber-300 mb-1">
                              AMESim data file format issue
                            </p>
                            {f.validation.error ? (
                              <p className="text-xs text-amber-200/70">{f.validation.error}</p>
                            ) : !f.validation.has_header ? (
                              <>
                                <p className="text-xs text-amber-200/70 mb-2">
                                  This file is missing the required AMESim table header. Without it,
                                  the FMU will fail during initialization with{' '}
                                  <span className="font-mono">"Undetermined format"</span>.
                                </p>
                                <p className="text-xs text-amber-200/70 mb-3">
                                  Detected: <span className="font-mono text-amber-200">{f.validation.n_points}</span> data points,{' '}
                                  <span className="font-mono text-amber-200">{f.validation.n_vars}</span> variable{f.validation.n_vars !== 1 ? 's' : ''} + time column.
                                  Clicking repair will insert the header line{' '}
                                  <span className="font-mono bg-amber-900/50 px-1.5 py-0.5 rounded text-amber-200">
                                    {f.validation.n_points}{'\t'}{f.validation.n_vars}
                                  </span>{' '}
                                  at the top of the file.
                                </p>
                                <button
                                  onClick={() => void handleRepairDataFile(f.name)}
                                  disabled={repairingFile === f.name}
                                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-amber-200 bg-amber-800/50 hover:bg-amber-800/80 disabled:opacity-50 rounded-lg transition-colors"
                                >
                                  {repairingFile === f.name ? (
                                    <>
                                      <div className="w-3 h-3 border-2 border-amber-500/40 border-t-amber-300 rounded-full animate-spin" />
                                      Repairing…
                                    </>
                                  ) : (
                                    <>
                                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17l-5.384-3.065A2.625 2.625 0 016 9.544V6.75a.75.75 0 01.75-.75h10.5a.75.75 0 01.75.75v2.794a2.625 2.625 0 01-.036 2.561l-5.384 3.065a2.625 2.625 0 01-2.58 0z" />
                                      </svg>
                                      Repair file
                                    </>
                                  )}
                    ...
                    {f.validation && f.validation.format_valid && f.validation.has_header && (
                      <div className="mt-1 px-3">
                        <p className="text-xs text-gray-600">
                          {f.validation.n_points} points, {f.validation.n_vars} variable{f.validation.n_vars !== 1 ? 's' : ''} + time column
                        </p>
                      </div>
                    )}
                  </div>
